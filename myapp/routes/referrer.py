...
from myapp.utils import can_upload_applicant_email, can_upload_applicant_phone, validate_file
...
@bp.route('/upload_by_referral/<int:referrer_id>', methods=['GET', 'POST'])
@no_cache
@login_required
@role_required('referrer')
def upload_by_referral(referrer_id):
    if '_user_id' not in session:
        current_app.logger.warning(f"Session expired for user {current_user.username}")
        return {'error': 'Session expired. Please log in again.'}, 401
    ...
    if request.method == 'GET':
        ...
        job_positions = JobRequirement.query.with_entities(JobRequirement.id, JobRequirement.position)\
                                            .filter(JobRequirement.for_vendor == True).all()
        if not form_data:
            form_data = {}
        return render_template(
            'referrer/upload_candidate.html',
    ...
    if not validate_file(file):
        ...
        return redirect(url_for('referrer.upload_by_referral', referrer_id=referrer_id))
    ...
    job_id = int_or_none(request.form.get('position'))
    ...
    try:
        ...
        db.session.add(new_referral)
        db.session.commit()
        flash('New referral applicant successfully created!', 'success')
        current_app.logger.info(f"New referral (Name = {new_referral.name.title()}) added by {current_user.username}")
        ...
        flash('Error creating referral. Please try again.', 'error')
        current_app.logger.error(f"Error creating referral: {str(e)}")
        return render_template('referrer/upload_candidate.html', form_data=request.form, job_positions=job_positions)